...
sub getOutput {
    ...
    if ($cmd eq 'store'){
        ...
        if ($self->hasFlag("all-trinkets")){
            ...
            return "All of your trinkets have been sold.";
        }
...
sub load {
    ...
    {	type => 'item',
	id => 'arthur',
	name => 'excalibur shovel',
	desc => 'Excal1bur, 6hovel of Kings from the dawn of time.',
	price => '10000',
	price_unit => 'pound',
	rarity_level => '1',
	group => 'shovel',
	...
	can_own_max => 1,
	for_sale => 1,
	attributes => { money_multiplier=> 16}
    },

    {	type => 'item',
	id => 'theforce',
	name => 'light saber shovel',
	desc => 'Dig or d1g not. There is no 7ry.',
	price => '150',
	price_unit => 'trilobite',
	rarity_level => '1',
	group => 'shovel',
	...
	can_own_max => 1,
	for_sale => 1,
	attributes => { money_multiplier=> 17}
    },
    ...
    {   type => 'trinket',
        id  => 'croix',
        name => 'a can of peach-pear La Croix',
        desc => 'a can of peach-pear La Croix',
        price => '1.15',
        price_unit => 'money',
        rarity_level => '1',
        group   => '',
    ...
    if ($lurecords[0]->{val2} < (time() - $self->{stock_update_freq} ) ){
        ...
        if ($sym_list){
            ...
            foreach my $line (@lines){
				...
				if (@records){
                    $self->{stock_c}->updateRecord($records[0]->{row_id}, {val4=>$price, val5=>time()});
                }else{
                    $self->{stock_c}->add('quote_'.$mode, $col[0], $col[1], $price, time());
                }
            }
            $self->{stock_c}->endBatch();
        }
...
sub lookupNewStock{
    ...
    # col1 = company name col2 = symbol col3 = price
